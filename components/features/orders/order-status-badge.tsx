...
const STATUS_CLASS_BY_STATUS = new Map<OrderStatus, string>([
...
export function OrderStatusBadge({ status, className }: OrderStatusBadgeProps) {
  const label = getStaffOrderStatusLabel(status);
  ...
  return (
    <span
      ...
      className={cn(
        ...
        STATUS_CLASS_BY_STATUS.get(status) ?? "bg-gray-100 text-gray-600",
        className,
      )}
